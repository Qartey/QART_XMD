...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const systemMessage = { role: 'system', content: 'You are called Qartde. Developed by Rahmani xmd. You respond to user commands. Only mention developer name if someone asks.' };
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const model = 'gpt-4-turbo-2024-04-09';
    const userMessage = { role: 'user', content: alpha };  
    const systemMessage = { role: 'system', content: 'You are called Qartde bot. Developed by Rahmani xmd bot. You respond to user commands. Only mention developer name if someone asks.' };
...
setInterval(async () => {
    if (conf.AUTO_BIO === "yes") {
        ...
        const bioText = `RAHMANI-XMD is online! 🚀\n${currentDateTime}`; // Format the bio text
...
zk.ev.on("messages.upsert", async (m) => {
    ...
    if (messageContent.slice(1).toLowerCase() === "vcf") {
        ...
        const baseName = "RAHMANI-XMD BOT";
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            console.log("\t🌍RAHMANI-XMD-BOT ONLINE🌍");
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'on')) {
            let msg = `*RAHMANI-XMD-BOT WELCOME MESSAGE*`;
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                console.log("✅ RAHMANI-XMD BOT Connected to WhatsApp! ☺️");
                console.log("--");
